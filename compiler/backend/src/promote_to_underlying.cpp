...
UnderlyingType promote_to_underlying(const Type& type) {
	...
	if (auto p = std::get_if<std::shared_ptr<TypeRotten>>(&type)) {
		return *p;
	}

	if (auto p = std::get_if<std::shared_ptr<TypeEnum>>(&type)) {
		return *p;
	}
	
	if (auto p = std::get_if<std::shared_ptr<TypeMapReference>>(&type)) {
		return *p;
	}
